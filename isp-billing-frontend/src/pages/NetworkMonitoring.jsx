...
import { 
    Activity, Server, Wifi, AlertTriangle,
    Terminal, TrendingUp, CheckCircle, Zap,
    Cpu, Clock, Users, Globe
} from 'lucide-react';

const CHART_CAPACITY = 100;
const POLL_INTERVAL  = 10_000; // 10 detik — cukup untuk NOC, MikroTik tidak kewalahan
...
const NetworkMonitoring = () => {
    ...
    // Ref untuk track apakah sedang ada request aktif
    // Mencegah request menumpuk (overlapping)
    ...
    const fetchLiveMonitor = useCallback(async () => {
        ...
        if (isFetchingRef.current) {
            console.debug('[NOC] Skip poll — request sebelumnya masih berjalan');
            return;
        }

        // Batalkan request sebelumnya kalau ada (defensive)
        ...
        try {
            const res = await axios.get('/api/noc/live', {
                // Pakai path relatif — tidak hardcode IP/port
                // Pastikan Vite proxy sudah dikonfigurasi (lihat bawah)
                signal:  abortController.current.signal,
                timeout: 9000, // timeout sedikit di bawah interval polling
            });

            if (res.data.success) {
                const { cpu_load, uptime, logs, traffic } = res.data.data;

                setNocData({ cpu_load, uptime, logs, is_demo: false });
                ...
                // Backend merespons tapi gagal (config missing, dsb)
                setError(res.data.message ?? 'Gagal mengambil data dari Router.');
            }

        } catch (err) {
            if (axios.isCancel(err) || err.name === 'CanceledError') {
                // Request sengaja dibatalkan — bukan error sebenarnya
                return;
            }
    ...
    useEffect(() => {
        ...
        // Polling interval — fetchLiveMonitor sudah guard overlap sendiri
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2 mt-3">
                                <div
                                    className={`h-2 rounded-full transition-all duration-500
                                        ${nocData.cpu_load > 80 ? 'bg-red-500' : 'bg-blue-500'}`}
                                    style={{ width: `${nocData.cpu_load}%` }}
                                />
                            </div>
                        </div>
                    </div>
